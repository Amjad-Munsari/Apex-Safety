...
### Production operational setup — not a platform or handover blocker
Verified counts: `form_templates` **0**, `clients` 0, `services` **0**, `contractors` 0, `proposals` 0. Matt is the only real row in `admin_users`, so the working screens correctly open empty until operational records are created.
- **No FRA / site-risk master template yet** — `seed.sql` does not seed templates. Matt can create or import the template he wants to use before the first assessment.
- **No service-catalogue rows yet** — proposals have no reusable line items until Matt adds or imports services. `seed.sql` contains 25 training courses and 10 services, but production was intentionally left empty rather than publishing potentially stale customer-facing prices. This is first-use setup, not unfinished platform work.
...
> **Historical status at HEAD `2e58cee`:** this note predates the completed signing fixes. Migrations 029 and 032 are now applied in production, and first-party signing is the approved live method. The current verification state is recorded in the 26 July sections below.
...
**Resolved after this finding:** the stamped PDF is written to a new content-addressed path, the original remains immutable, both hashes are retained, and migration 032 commits the token, proposal status, signed-file link, and evidence row atomically. First-party signing is the live product method.
...
Before this fix, `lib/notifications/client-form-events.ts` logged a failed dispatch to `console.error` and nothing else — no `workflow_errors` row, unlike the other dispatch callers. At that point the downstream workflow could also return an empty `200` after a failed mail step, so Matt's Workflow Errors page could read “ALL CLEAR” while `client_form_created`, `client_form_submitted`, or `client_template_cloned` had failed. The wrapper now attempts a best-effort error-row write after failed transport or terminal-receipt checks, and the rebuilt live workflow returns success only after Gmail acceptance (§2C).

### FIXED (commit `49c1f57`) — outbound n8n webhook had no timeout while awaited on the client's submit path
Before this fix, `dispatchToN8n` called `fetch` with no `AbortSignal` or timeout while `dispatchClientFormEvent` was awaited inline in the client actions, so an n8n endpoint that accepted the connection but never answered could hold the customer's submit spinner until the platform timeout. The database write was already committed, so the fix added an eight-second outbound budget and attempts a best-effort error-row write without undoing the client's action.
...
- **Identity aligned (2026-07-26):** Merlin Safety System is the platform and transactional-email brand. 888 Safety & Training remains the consultancy/service-provider identity on proposals, reports, contracts, and PayPal. All public contact surfaces and Reply-To values use `info@888safetyandtraining.com` and `0333 049 8979`.
- **Live n8n cutover verified (2026-07-26):** three workflows are active: `Email Notifications` (`hif6MMPvywQF6z6u`), `Assessment Report Notifications` (`eijErYNTCnWHuITQ`), and `Merlin Automation Failure Alerts` (`fzflrF6ByBnfRhxN`). Both webhooks authenticate, reject unauthenticated calls with `403`, validate their exact four admin-notice events, retry Gmail three times, return success only after Gmail acceptance, reject invalid events with `422`, and retain no webhook payloads. The eight obsolete customer-email branches are gone. The matching general and assessment secrets and both production webhook URLs are deployed in Vercel, the general n8n credential was rotated to the same new value, and production deployment `dpl_BYiyBL1eHxVs3jhbT9Q9M7jgNGG9` is Ready and aliased to `www.merlinsafetysystem.com`. At `2026-07-25T21:34:25Z`, controlled payloads matching `client_form_created`, `client_form_submitted`, `client_template_cloned`, and the Matt-submitted assessment notice each returned `200` with `{ ok: true, delivered: true }` after Gmail accepted the message. A controlled source failure had already produced the separate alert email. Historical executions `1` and `2`, which retained the old general secret, were then deleted and confirmed absent. Exact definitions and the deploy script are under `docs/n8n/workflows/` and `scripts/n8n/`. Recommended non-blocking post-handover security housekeeping: rotate the shared n8n audit key, enable MFA, confirm/remove two unused mail credentials, and take the available `2.31.6` bug-fix update.
...
- **n8n API access (UPDATED 2026-07-26):** `.env.local` contains `N8N_API_KEY` for `https://fire-safety.app.n8n.cloud/api/v1`; load it without printing it and never commit it. The key was shared in chat and expires at **2026-08-23 18:30 UTC**, so rotate it now that the webhook cutover is complete and replace the ignored local value. Pre-rebuild workflow backups are mode-`600` files under `/private/tmp/fire-safety-n8n-backup-20260726/`. The old secret-bearing executions `1` and `2` have been deleted; execution `5` is the retained controlled failure-alert evidence and contains no webhook request secret.
