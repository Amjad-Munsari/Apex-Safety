...
- **Live n8n rebuild (2026-07-26):** three workflows are active: `Email Notifications` (`hif6MMPvywQF6z6u`), `Assessment Report Notifications` (`eijErYNTCnWHuITQ`), and `888 Automation Failure Alerts` (`fzflrF6ByBnfRhxN`). The two webhooks now authenticate, validate their exact four admin-notice events, retry Gmail three times, return success only after Gmail acceptance, reject invalid events with `422`, and retain no webhook payloads. The eight obsolete customer-email branches are gone. Assessment auth/valid/invalid canaries passed, and a controlled source failure produced a successful alert email; temporary canary records were removed. Exact definitions and deploy script are under `docs/n8n/workflows/` and `scripts/n8n/`. Application commit `559b80df4047dbc8fc8d2adcb4cd720bca2d3f58` is deployed to production. Remaining: complete the Vercel two-secret cutover, run the four application-to-n8n notice canaries, rotate the old general secret plus shared API key, delete only historical executions `1` and `2`, enable MFA, confirm/remove two unused mail credentials, and take the available `2.31.6` bug-fix update.
...
- **n8n API access (UPDATED 2026-07-26):** `.env.local` contains `N8N_API_KEY` for `https://fire-safety.app.n8n.cloud/api/v1`; source it with `set -a`, never print it, and never commit it. The key was shared in chat and expires at **2026-08-23 18:30 UTC**, so rotate it after the Vercel/general-secret cutover and replace the ignored local value. Pre-rebuild workflow backups are mode-`600` files under `/private/tmp/fire-safety-n8n-backup-20260726/`; runtime secrets are mode-`600` under `/private/tmp/fire-safety-n8n-runtime-20260726/` and are not durable handover storage. Historical executions `1` and `2` expose only the old general secret and should be deleted after that secret is rotated at both ends.
