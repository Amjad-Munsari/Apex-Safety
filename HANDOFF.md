...
- **Live n8n audit (2026-07-25):** exactly two workflows are active. `Email Notifications` (`hif6MMPvywQF6z6u`) has three current `client_form_*` branches plus eight obsolete customer-email branches. Its protected webhook is receiving production events, but retained executions `1` (form created) and `2` (form submitted) both failed because the three current Gmail nodes use the invalid literal recipient `888 / Matt`; there is also no `client_id` enrichment, retry, or error workflow. `Assessment Report Notifications` (`eijErYNTCnWHuITQ`) only emails the submission ID/time to `888FST@proton.me`; its live webhook is unauthenticated, while the application blocks its own calls because `N8N_ASSESSMENT_WEBHOOK_SECRET` is absent from the 25 July Vercel snapshot. Both webhooks acknowledge before Gmail finishes, so downstream failures are invisible to the application's Workflow Errors page. Full evidence and safe QA gates are in `docs/888-manual-research.md`.
...
- **n8n API access (ADDED 2026-07-25):** `.env.local` now contains `N8N_API_KEY` for workflow and execution audits against `https://fire-safety.app.n8n.cloud/api/v1`. The key is gitignored, expires at **2026-08-23 18:30 UTC**, and must never be committed or copied into documentation. It was shared in chat, and the retained failed executions expose the live `X-Webhook-Secret` unredacted to that key. After this audit, rotate both the n8n API key and `N8N_WEBHOOK_SECRET` in the n8n header-auth credential and Vercel production, then replace the ignored local API-key value.
