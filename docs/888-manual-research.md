...
| n8n sends all customer email. | Ten transactional email types are sent directly through Resend. n8n sends four internal notices to Matt: client template created, client form submitted, client template customised, and Matt-led assessment submitted. | **Live application code and n8n workflows; production-secret cutover pending:** both webhooks now authenticate, validate their event, retry Gmail up to three times, and return success only after Gmail accepts the message. The deployed application requires that final receipt and records anything else in Workflow Errors. `lib/notifications/email-templates.ts:21-33`; `lib/notifications/dispatch.ts:235-309`; `app/admin/assessments/actions.ts:431-471`; `docs/n8n/workflows/email-notifications.json:1-118`; `docs/n8n/workflows/assessment-report-notifications.json:1-125` |
...
The assessment n8n webhook is separate from AI generation and fires only when Matt submits through the admin assessment action. The application posts `{ submissionId }` with a bearer secret, allows 15 seconds for the workflow's bounded retries, requires the exact final receipt `{ ok: true, delivered: true }`, and records a workflow error for a missing secret, rejection, timeout, empty success, or network failure (`app/admin/assessments/actions.ts:431-471`; `lib/notifications/dispatch.ts:238-267`). The live n8n workflow authenticates the request, rejects a missing ID with `422`, retries Gmail up to three times, sends one fixed admin email to `888FST@proton.me`, and returns its success receipt only after Gmail accepts the send (`docs/n8n/workflows/assessment-report-notifications.json:1-125`; `docs/n8n/workflows/assessment-report-notifications.json:127-178`). It does not query the submission, contact a client, generate AI content, create a PDF, or back up to Drive.
...
Customer-template self-fill stores the answers, attempts the client-form activity event, and schedules the same AI draft as assigned and Matt-led work (`app/client/templates/actions.ts:481-520`; `lib/reports/report-draft.ts:140-164`). The event includes the authoritative organisation name. The live n8n workflow validates the event and returns success only after its admin email is accepted; an invalid event returns `422`, while any missing final receipt is recorded in Workflow Errors by the application (`lib/auth-helpers.ts:179-186`; `lib/notifications/dispatch.ts:106-132`; `lib/notifications/dispatch.ts:238-309`; `docs/n8n/workflows/email-notifications.json:119-350`; `docs/n8n/workflows/email-notifications.json:351-474`).
...
|---|---|---|
| `Email Notifications` (`hif6MMPvywQF6z6u`) | Accepts only `client_form_created`, `client_form_submitted`, and `client_template_cloned`; validates the organisation and event fields; sends the matching plain-text admin message to `888FST@proton.me`; and rejects unknown or incomplete events with `422`. The eight obsolete customer-email branches are gone. | Header authentication is required. Each Gmail send has three attempts with a one-second delay. The `200` receipt is downstream of Gmail and contains `{ ok: true, delivered: true }`. Success and error payloads are not retained in n8n. `docs/n8n/workflows/email-notifications.json:1-118`; `docs/n8n/workflows/email-notifications.json:119-350`; `docs/n8n/workflows/email-notifications.json:351-474`; `docs/n8n/workflows/email-notifications.json:574-617` |
| `Assessment Report Notifications` (`eijErYNTCnWHuITQ`) | Validates `submissionId`, sends one admin message with the review link, returns the same final delivery receipt, and returns `422` for an invalid event. It still does not generate the report, contact the client, or store a file. | Bearer-header authentication is required. An unauthenticated call returned `403`; a valid controlled call returned `200` with the terminal receipt and Gmail accepted the message; an authenticated empty payload returned `422`. Three Gmail attempts and no success/error payload retention are configured. `docs/n8n/workflows/assessment-report-notifications.json:1-125`; `docs/n8n/workflows/assessment-report-notifications.json:127-178` |
...
The application event contract was changed with the workflows. Client events now carry the authoritative organisation name returned by the signed-in organisation lookup, and all three call sites send it (`lib/auth-helpers.ts:179-186`; `lib/notifications/dispatch.ts:106-132`; `app/client/templates/actions.ts:75-84`; `app/client/templates/actions.ts:506-515`; `app/client/assignments/actions.ts:354-362`; `app/client/assignments/actions.ts:525-536`). Both application paths reject an ordinary `2xx` that lacks the exact final delivery receipt, so n8n's known “empty `200` after an internal failure” behaviour becomes an application-side Workflow Error rather than a silent success (`lib/notifications/dispatch.ts:238-309`; `app/admin/assessments/actions.ts:431-471`).
...
Two historical failed executions, `1` and `2`, still contain the old general webhook secret in retained request headers. The owner API key was shared in chat and can read those records. The safe cutover is therefore one operation: set a new general secret in production, update the existing n8n header credential to the same value, prove all three valid event branches, delete only executions `1` and `2`, rotate the n8n API key, and replace the ignored local key. The current application contract is deployed, but the three application-to-n8n notices remain **Staged (not live)** until the exposed secret is rotated and all three production event branches pass controlled canaries.
...
| Public assessment webhook | **Resolved in live n8n.** Unauthenticated requests return `403`; invalid authenticated events return `422`. Production application-secret deployment remains the final cutover gate. |
...
| General secret exposed through retained failures; audit API key shared | **Open until coordinated rotation.** Rotate both secrets and delete only the two historical executions after the new general secret is live at both ends. |
...
| Client-form activity notices | Client create, submit, and fork actions send the organisation name and record references to the authenticated general n8n webhook after the database write (`lib/notifications/client-form-events.ts:28-72`; `app/client/templates/actions.ts:51-86`; `app/client/assignments/actions.ts:331-367`). | **Live code and n8n; held for secret rotation and canaries.** The rebuilt workflow validates, retries, and confirms terminal Gmail delivery. The database write remains committed if the notice fails, and the application records the failed receipt. The three valid production event canaries remain **UNVERIFIED** until the coordinated general-secret cutover. |
| Matt-submitted assessment notice | Only the admin submit action schedules the separate authenticated n8n POST containing `{ submissionId }`; client submissions do not call it (`app/admin/assessments/actions.ts:431-471`; `app/client/assignments/actions.ts:354-367`; `app/client/templates/actions.ts:506-520`). | **Live code and n8n; held for the production secret.** Authentication, rejection, final receipt, Gmail delivery, and failure alerting passed direct controlled canaries. The matching production application secret still has to be deployed before the application path can be proved. |
...
| Workflow Errors | Application-side failures are inserted into `workflow_errors`; Matt sees the 50 newest rows (`lib/supabase/dashboard.ts:402-417`; `app/admin/errors/page.tsx:8-74`). n8n success is accepted only with its terminal delivery receipt (`lib/notifications/dispatch.ts:238-309`). | **Partial.** A database read failure is converted to an empty list, so the UI can say “No operational errors detected” when the log could not be read. Only seven of at least 22 written workflow names have tailored explanations (`lib/workflow-errors.ts:20-65`). The separate n8n failure workflow emails Matt when an n8n execution itself fails. |
...
- The general three-event canary is paused until the old exposed secret is rotated in production and n8n together. Do not create duplicate client records merely to test an uncompleted cutover.
...
| n8n | Three client-form activity notices and a separate Matt-submitted assessment notice. It does not generate AI content, PDFs, or ordinary customer email. | **Live application code and workflows; production-secret cutover pending:** both endpoints authenticate, validate, retry Gmail, alert on workflow failure, and return success only after delivery acceptance. Direct assessment canaries passed; the two production application secrets and three general-event canaries remain pending. | `lib/notifications/client-form-events.ts:28-72`; `lib/notifications/dispatch.ts:238-309`; `app/admin/assessments/actions.ts:431-471`; `docs/n8n/workflows/automation-failure-alerts.json:1-67` |
...
| n8n | The live workflow rebuild is complete and the assessment plus controlled failure canaries passed. The application source now sends `client_name` and requires terminal Gmail confirmation. Production still needs the coordinated assessment/general secret cutover and the three general-event canaries. Historical executions `1` and `2` must be deleted after the general secret rotates; MFA, API-key rotation, two unused mail credentials, and the hosted bug-fix update remain account-level work (`lib/notifications/dispatch.ts:238-309`; `app/admin/assessments/actions.ts:431-471`; see **Live n8n rebuild and audit**). |
...
| Deploy the two matching application secrets, rotate the exposed general secret, and run one controlled canary for each of the four admin notices | Dependable client activity and Matt-submitted assessment notices | **Application release and n8n rebuild are live; secret cutover and four application-to-n8n canaries remain pending. Direct assessment and failure-alert canaries passed.** |
| Delete historical executions `1` and `2` after secret rotation, rotate the n8n API key, enable MFA, remove confirmed-unused mail credentials, and apply the available bug-fix update | Close the remaining partner-automation credential and account exposure | **Required before handover; no webhook is now public, but the old retained executions still expose the old general secret to the shared API key.** |
