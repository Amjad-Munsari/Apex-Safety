...
**Source basis:** `main` at `62b16d7`, plus the fix set documented below; deployment of that fix set is unverified
**Production URL recorded in the latest handoff:** `https://www.merlinsafetysystem.com`

This brief is the evidence layer for `docs/888-user-testing-manual.md`. It was derived from the current application code, migrations, configuration, current uncommitted files, and the latest production handoff. The fixes described as **Staged (not live)** are present in the working tree but are not treated as deployed. Earlier user guides and proposal documents were not used as behavioural sources. The 20 July production-readiness audit was used only as a checklist, then every finding was rechecked against the current checkout.
...
| The application needs SMTP to send its account and workflow email. | The current account actions generate invite/recovery links themselves and dispatch them through Resend, which also sends the other application emails. SMTP is not the active application transport. | **Partial:** verify the Resend sender domain, SPF/DKIM/DMARC alignment, Reply-To, and real inbox delivery. `app/admin/clients/actions.ts:420-455`; `app/admin/clients/actions.ts:458-562`; `app/login/forgot/actions.ts:42-72`; `lib/notifications/dispatch.ts:180-219` |
...
The active Select renderer accepts one choice. The fix set also corrects the palette description to “Single choice from list” (`components/form-builder/field-palette.tsx:41-48`; `components/form-interpreter/select-field-renderer.tsx:39-60`).
...
| `/client/assignments/[id]/fill` | Resumable fill using the assignment's pinned template version. The fix set adds signed previews and complete photo removal. | **Live route; photo fix staged** | `app/client/assignments/actions.ts:98-178`; `app/client/assignments/[id]/fill/page.tsx:21-61`; `components/form-interpreter/multi-photo-field-renderer.tsx:150-173`; `components/form-interpreter/multi-photo-field-renderer.tsx:282-303` |
...
The client-to-report handoff defect is fixed in the working tree: both client submission paths call the shared report scheduler after the `submitted` write succeeds. This is **Staged (not live)** until the fix set is deployed; production should not be retested against this expectation before that deployment (`app/client/assignments/actions.ts:331-366`; `app/client/templates/actions.ts:475-508`; `lib/reports/report-draft.ts:140-164`).

The assessment n8n webhook is separate from AI generation. The fix set posts `{ submissionId }` with a bearer secret to `N8N_ASSESSMENT_WEBHOOK_URL`, checks the HTTP result, and records a workflow error for a missing secret, non-success response, timeout, or network failure. It is **Staged (not live)** until the application and n8n are deployed with the same secret (`app/admin/assessments/actions.ts:424-471`; `.env.example:38-45`).
...
Customer-template self-fill stores the answers, sends the partner form event, and, in the fix set, schedules the same AI draft as assigned and Matt-led work. The handoff is **Staged (not live)** until deployment (`app/client/templates/actions.ts:475-508`; `lib/reports/report-draft.ts:140-164`).
...
- The fix set leaves the Draft for retry but now fails visibly when proposal PDF generation or upload fails, so the builder no longer reports a successful send without a document (`app/admin/proposals/actions.ts:345-381`; `components/proposals/advanced-proposal-builder.tsx:210-240`).
- A proposal still becomes Sent before delivery is known. The fix set now records a signature-email failure in Workflow Errors and shows Matt a warning with the proposal link; there is still no automatic retry queue (`app/admin/proposals/actions.ts:195-222`; `components/proposals/advanced-proposal-builder.tsx:229-239`).
...
| n8n | Three partner form events and a separate assessment-submitted event. It does not generate AI content, PDFs, or ordinary customer email. | **Partial:** current downstream form-event workflow is broken after accepting the request. Authenticated assessment-webhook handling is staged for deployment and still needs matching configuration. | `lib/notifications/client-form-events.ts:28-72`; `app/admin/assessments/actions.ts:424-471`; `HANDOFF.md:141-144` |
...
| Submission/report | Work can move `draft` → `submitted` → `draft_ready_for_review` → `completed`; `ai_draft_failed` is the retry state. The fix set connects Matt-led, client-assigned, and customer self-fill submissions to the same draft scheduler. Some client screens collapse the middle states to In Progress. | `app/admin/month-summary/page.tsx:101-123`; `app/client/assessments/status.ts:19-40`; `lib/reports/report-draft.ts:96-136`; `app/client/assignments/actions.ts:331-366` |
...
| Client onboarding | The fix set adds Site Address to New Client and an Edit details control for organisation name, primary contact, email, phone, and address. Job title is still not modelled, so Sarah's “Facilities Manager” title remains test context rather than stored account data (`components/clients/new-client-dialog.tsx:23-56`; `components/clients/new-client-dialog.tsx:101-135`; `app/admin/clients/actions.ts:84-123`; `app/admin/clients/[id]/page.tsx:318-340`). |
...
| n8n | Client-form events are sent, but the downstream workflow currently errors because it does not resolve `client_id`. The assessment webhook is hardened locally but requires the same bearer secret in both deployments (`HANDOFF.md:141-144`; `app/admin/assessments/actions.ts:424-471`). |
...
| Client assignment to report | **Staged fix:** assigned-form and customer self-fill submissions now schedule report drafting after the committed submission. Deploy before testing this as live (`app/client/assignments/actions.ts:331-366`; `app/client/templates/actions.ts:475-508`). |
| Proposal delivery | **Staged fix:** PDF failure now fails visibly while preserving the Draft, and signature-email failure creates a Workflow Error plus an on-screen warning. There is still no automatic retry queue (`app/admin/proposals/actions.ts:345-399`; `app/admin/proposals/actions.ts:195-222`). |
...
| Photos | **Staged fix:** committed photos use 15-minute signed previews and removal deletes both the private object and its audit row. A Photos field marked Required is still intentionally presented as “recommended” and does not block submission (`app/admin/assessments/actions.ts:648-724`; `components/form-interpreter/multi-photo-field-renderer.tsx:150-173`; `components/form-interpreter/multi-photo-field-renderer.tsx:282-334`). |
...
| Password reset | **Staged fix:** the public action keeps the same response but now applies hashed per-account (3/hour) and per-IP (20/hour) counters. It fails open if the limiter itself is unavailable. Migration 030 is not confirmed in production, so the protection is not live until migration and application deployment (`app/login/forgot/actions.ts:18-40`; `lib/rate-limit.ts:22-59`; `supabase/migrations/030_rate_limit.sql:32-99`). |
...
| Job-title storage decision | A saved role/title for contacts such as “Facilities Manager” | **Not modelled; site address and profile editing are staged for deployment.** |
...
| n8n workflow fix for `client_id`, plus matching assessment-webhook secret configuration | Dependable partner automation | **Downstream mapping remains broken; app-side authentication is staged for deployment.** |
| Deploy the client-submission report handoff | Put client-completed assessments into Matt's visible drafting/review workflow | **Connected in the local source; deployment not confirmed.** |
...
| Apply migration 030 and deploy the password-reset limiter | Reduces reset-email flooding without exposing whether an account exists | **Code integrated; production migration/deployment not confirmed.** |
