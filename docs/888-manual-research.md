...
| n8n sends all customer email. | Transactional email is sent directly through Resend. n8n is only the destination for three client-form events and a separate assessment-submitted event for partner automation. | **Live email transport; held partner automation:** email delivery is owner-confirmed operational as of 25 July 2026. Both production n8n secrets are empty or absent, so the platform currently blocks all four partner events. The form-event workflow also has an unresolved `client_id` lookup after it is re-enabled. `.env.example:28-44`; `lib/notifications/dispatch.ts:180-283`; `HANDOFF.md:141-144` |
...
The assessment n8n webhook is separate from AI generation. The deployed code posts `{ submissionId }` with a bearer secret to `N8N_ASSESSMENT_WEBHOOK_URL`, checks the HTTP result, and records a workflow error for a missing secret, non-success response, timeout, or network failure. Production deliberately blocks this event because `N8N_ASSESSMENT_WEBHOOK_SECRET` is missing; the event can only be enabled after the same secret is configured and enforced in n8n (`app/admin/assessments/actions.ts:424-471`; `.env.example:38-45`).
...
Customer-template self-fill stores the answers, sends the partner form event, and schedules the same AI draft as assigned and Matt-led work (`app/client/templates/actions.ts:475-508`; `lib/reports/report-draft.ts:140-164`).
...
| n8n | Production currently blocks all partner events: `N8N_WEBHOOK_SECRET` is empty and `N8N_ASSESSMENT_WEBHOOK_SECRET` is absent. After matching secrets are configured and enforced in n8n, the form-event workflow still needs its `client_id` recipient lookup fixed (`lib/notifications/dispatch.ts:225-267`; `app/admin/assessments/actions.ts:424-471`; `HANDOFF.md:141-144`). |
...
| n8n workflow fix for `client_id`, plus matching secrets for both partner webhook paths | Dependable partner automation | **Both partner paths are blocked by empty/missing production secrets; downstream form-event mapping also remains broken.** |
