...
| n8n sends all customer email. | Ten transactional email types are sent directly through Resend. n8n sends four internal notices to Matt: client template created, client form submitted, client template customised, and Matt-led assessment submitted. | **Live n8n workflows; application release pending final production-secret cutover:** both webhooks now authenticate, validate their event, retry Gmail up to three times, and return success only after Gmail accepts the message. The application source requires that final receipt and records anything else in Workflow Errors. `lib/notifications/email-templates.ts:21-33`; `lib/notifications/dispatch.ts:235-309`; `app/admin/assessments/actions.ts:431-471`; `docs/n8n/workflows/email-notifications.json:1-118`; `docs/n8n/workflows/assessment-report-notifications.json:1-125` |
...
Two historical failed executions, `1` and `2`, still contain the old general webhook secret in retained request headers. The owner API key was shared in chat and can read those records. The safe cutover is therefore one operation: set a new general secret in production, update the existing n8n header credential to the same value, prove all three valid event branches, delete only executions `1` and `2`, rotate the n8n API key, and replace the ignored local key. Until that cutover finishes, the new general workflow is live but the three application-to-n8n notices are **Staged (not live)** because the deployed application still sends the earlier payload contract.
...
| Client-form activity notices | Client create, submit, and fork actions send the organisation name and record references to the authenticated general n8n webhook after the database write (`lib/notifications/client-form-events.ts:28-72`; `app/client/templates/actions.ts:51-86`; `app/client/assignments/actions.ts:331-367`). | **Live n8n; staged application cutover.** The rebuilt workflow validates, retries, and confirms terminal Gmail delivery. The database write remains committed if the notice fails, and the application records the failed receipt. The three valid production event canaries remain **UNVERIFIED** until the coordinated general-secret cutover. |
| Matt-submitted assessment notice | Only the admin submit action schedules the separate authenticated n8n POST containing `{ submissionId }`; client submissions do not call it (`app/admin/assessments/actions.ts:431-471`; `app/client/assignments/actions.ts:354-367`; `app/client/templates/actions.ts:506-520`). | **Live n8n; staged application cutover.** Authentication, rejection, final receipt, Gmail delivery, and failure alerting passed controlled canaries. The matching production application secret still has to be deployed. |
...
| n8n | Three client-form activity notices and a separate Matt-submitted assessment notice. It does not generate AI content, PDFs, or ordinary customer email. | **Live workflows; staged application cutover:** both endpoints authenticate, validate, retry Gmail, alert on workflow failure, and return success only after delivery acceptance. Assessment canaries passed; the production application secrets and three general-event canaries remain pending. | `lib/notifications/client-form-events.ts:28-72`; `lib/notifications/dispatch.ts:238-309`; `app/admin/assessments/actions.ts:431-471`; `docs/n8n/workflows/automation-failure-alerts.json:1-67` |
...
| Deploy the two matching application secrets, rotate the exposed general secret, and run one controlled canary for each of the three client-form events | Dependable client activity and Matt-submitted assessment notices | **n8n rebuild complete; production application cutover pending. Assessment and failure-alert canaries passed.** |
