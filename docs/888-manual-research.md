...
| n8n sends all customer email. | Ten transactional email types are sent directly through Resend. n8n has two different internal-notification paths: three client-form activity events and one Matt-submitted assessment event. | **Live email transport; broken/unsafe n8n paths:** owner-confirmed Resend delivery is operational. The authenticated form-event webhook is receiving production calls, but every recorded execution failed on an invalid literal recipient. The assessment workflow is active and publicly callable without authentication, while the application currently blocks its own calls because the production snapshot has no `N8N_ASSESSMENT_WEBHOOK_SECRET`. `lib/notifications/email-templates.ts:21-33`; `lib/notifications/dispatch.ts:238-297`; `app/admin/assessments/actions.ts:428-475`; see **Live n8n audit** below. |
...
The assessment n8n webhook is separate from AI generation and fires only when Matt submits through the admin assessment action. The application posts only `{ submissionId }`, adds a bearer secret, waits up to three seconds for the initial HTTP response, and records a workflow error for a missing secret, non-success response, timeout, or network failure (`app/admin/assessments/actions.ts:428-475`). The live n8n workflow does much less than older comments claim: it copies the ID, fills a timestamp, and sends one fixed admin email to `888FST@proton.me`. It does not query the submission, contact a client, generate AI content, create a PDF, or back up to Drive. Its live webhook has no authentication, while the 25 July production environment snapshot contains the matching URL but no `N8N_ASSESSMENT_WEBHOOK_SECRET`; the application therefore blocks its own event, yet anyone who knows the n8n webhook URL can still trigger the admin email directly. Setting an application secret alone will not fix that exposure because n8n must enforce the same header.
...
Customer-template self-fill stores the answers, attempts the client-form activity event, and schedules the same AI draft as assigned and Matt-led work (`app/client/templates/actions.ts:478-518`; `lib/reports/report-draft.ts:140-164`). The live n8n webhook accepts the event before its email node runs, so the client's submission can succeed even when the downstream notification later fails.
...
### Live n8n audit — 25 July 2026

The n8n facts below came from a read-only query of the official public API for the live workspace. They are live-cloud evidence rather than repository inferences; workflow IDs and execution IDs are recorded so the check can be repeated without exposing webhook paths or credentials.

| Live workflow | What it actually contains | Authentication and recovery | Recorded execution state |
|---|---|---|---|
| `Assessment Report Notifications` (`eijErYNTCnWHuITQ`) | POST webhook → copy `submissionId`, optional template name, and timestamp → Gmail message to `888FST@proton.me`. Active version 5; last updated 30 June 2026. | The webhook has **no authentication**. The Gmail node has no retry, continue-on-fail, or error workflow. The webhook acknowledges on receipt rather than after Gmail completes. | No execution is retained in the live history. The connected Gmail credential is named `Info@ Account`, but a successful send through this workflow is **UNVERIFIED**. |
| `Email Notifications` (`hif6MMPvywQF6z6u`) | POST webhook → Switch with 11 branches. Eight are obsolete customer-email branches; three are current client activity notices: template created, form submitted, and template cloned. Active version 283; last updated 30 June 2026. | Header authentication is enabled through the `888 Webhook Secret` credential. Every Gmail node has no retry, continue-on-fail, or error workflow. The webhook acknowledges on receipt, so the application cannot see a later Gmail-node failure. | Exactly two executions are retained, both errors: execution `1` failed at **Client Form Created** and execution `2` failed at **Client Form Submitted**. Both say the literal To value `888 / Matt` is not a valid email address. No successful execution is retained. The untested clone branch has the same invalid To value. |

The live request bodies confirm that production sends only the fields defined in the current client-form event union: `client_id` plus the template/submission identifiers and timestamp. No client name or email is present (`lib/notifications/dispatch.ts:106-132`; `app/client/templates/actions.ts:75-83`; `app/client/templates/actions.ts:505-513`; `app/client/assignments/actions.ts:354-361`). The three n8n email bodies expect a client name, but the workflow has no Supabase lookup or other enrichment node. Fixing the To address alone would make the messages send with a blank client name.

The production environment snapshot pulled on 25 July contains names for `N8N_WEBHOOK_URL`, `N8N_WEBHOOK_SECRET`, and `N8N_ASSESSMENT_WEBHOOK_URL`; protected values are blank in that export. The two live form-event executions prove the general URL and secret are working in the deployed application. The assessment URL is visible and matches the active workflow, while `N8N_ASSESSMENT_WEBHOOK_SECRET` is absent from the snapshot. That is why the application deliberately does not send the admin-assessment event (`app/admin/assessments/actions.ts:440-453`). The live n8n workflow would not validate the bearer header even if the variable were added, because its webhook is currently unauthenticated.

The n8n-native security audit was then run through the owner-only public API. It independently flags the assessment webhook as unprotected and identifies two unused Gmail OAuth credentials that are not used by any workflow. It reports no database-query, filesystem, community-node, custom-node, or risky built-in-node finding. The workspace has one user and one personal project; that user's MFA flag is off. Both workflows have no description or tags. The instance reports version `2.31.5` with the bug-fix update `2.31.6` available; the audit does not label that update as a security fix.

The retained JSON for failed executions `1` and `2` contains the incoming `X-Webhook-Secret` as an unredacted 64-character value at the Webhook and downstream run-data nodes. The owner API key can read those records. The API key was shared in chat, is stored only in ignored `.env.local`, expires on 23 August 2026, and must be rotated after this review. Because that key could retrieve the live general webhook secret, `N8N_WEBHOOK_SECRET` must also be rotated in both the n8n header-auth credential and the production application environment; rotating only the API key is insufficient (`HANDOFF.md:149-150`).

The eight obsolete customer-email branches are structurally inconsistent with the current application even before their duplication risk is considered. In particular, the old portal-invite node reads `client_email`, but the real payload field is `recipient_email` (`lib/notifications/dispatch.ts:89-98`). All three current client-form messages read a top-level `client_name`, although the webhook envelope contains only `body.client_id` and the workflow has no enrichment step. The invalid To value prevents the recorded executions from reaching message evaluation, so date-expression behaviour in those nodes remains **UNVERIFIED**. Source comments still say that eight payload types are email, but the registered set now contains ten; the executable set is authoritative (`lib/notifications/dispatch.ts:8-15`; `lib/notifications/email-templates.ts:21-33`).

### Severity-ranked n8n findings

No finding is rated Critical because the exposed assessment webhook has no database or file node and only targets Matt's fixed inbox. That does not make the current setup production-ready: the High findings below can lose operational notices silently, and both exposed credentials must be rotated.

| Severity | Finding and consequence | Required correction |
|---|---|---|
| **High** | The current client-form automation is functionally broken. Both retained production executions failed at the Gmail node, all three current branches share the invalid `888 / Matt` recipient, and none can resolve `client_id` to a client name. The client write succeeds, so Matt receives no activity notice and the portal gives no warning. | Replace the recipient, add an explicit client lookup or send the required display fields, then validate created, submitted, and cloned events with one controlled execution each. |
| **High** | Both webhooks acknowledge before Gmail completes, and neither workflow has retry, continue-on-fail, or an error workflow. A later mail failure therefore looks successful to the caller and never reaches the application's Workflow Errors page (`lib/notifications/dispatch.ts:238-297`; `lib/notifications/client-form-events.ts:28-72`). | Respond only after the terminal step or add a durable error callback/queue, then alert on failed n8n executions and provide a controlled replay path. |
| **High** | The owner-level n8n API key was shared in chat while the sole workspace user has MFA disabled. It can read retained execution data, and those records contain the live `X-Webhook-Secret` unredacted. Exposure of the API key therefore also exposes the application's general n8n webhook secret. | Rotate the API key, rotate `N8N_WEBHOOK_SECRET` in both n8n and production, replace the ignored local API-key value, and enable MFA on the sole n8n account. |
| **Medium** | Assessment-notice authentication is contradictory: the application refuses to call without `N8N_ASSESSMENT_WEBHOOK_SECRET`, but the live workflow accepts unauthenticated requests. The result is no legitimate notice from the application and a public endpoint that can spam Matt's fixed inbox (`app/admin/assessments/actions.ts:428-475`). | Enable header authentication in n8n first, set the exact matching production secret, then run one controlled admin-assessment test. |
| **Medium** | Eight active Switch branches duplicate emails that the application now sends through Resend. The stale portal-invite branch also reads the wrong recipient field, and the repository verification script can still fire every obsolete branch (`lib/notifications/email-templates.ts:21-33`; `lib/notifications/dispatch.ts:89-98`; `scripts/verify-n8n.ps1:1-37`; `scripts/verify-n8n.ps1:59-160`). | Disable or remove the obsolete branches and replace the production-firing script with read-only inspection plus explicit, gated canary tests. |
| **Medium** | Two unused Gmail OAuth credentials remain stored in the workspace. They add recoverable credential exposure without supporting any active workflow. | Confirm ownership, revoke the unused OAuth grants, then remove the unused n8n credentials. |
| **Low** | The instance is one bug-fix release behind, and both workflows lack tags and descriptions. This does not explain the current failures, but it makes ownership and safe maintenance harder. | Take a workflow export, apply the `2.31.6` update in a controlled window, and add owner/purpose/runbook metadata. |
...
| Client-form activity notices | Client create, submit, and fork actions call the authenticated general n8n webhook after the database write (`lib/notifications/client-form-events.ts:28-72`; `app/client/templates/actions.ts:51-86`; `app/client/assignments/actions.ts:324-366`). | **Broken downstream.** The webhook accepts production calls, but all three n8n mail nodes have an invalid recipient and missing client enrichment. The client action remains successful by design. |
| Matt-submitted assessment notice | Only the admin submit action schedules the separate n8n POST containing `{ submissionId }`; client submissions do not call it (`app/admin/assessments/actions.ts:421-475`; `app/client/assignments/actions.ts:354-366`; `app/client/templates/actions.ts:505-518`). | **Blocked in the application and exposed in n8n.** The required application secret is absent, while the live webhook itself is public. No retry or manual resend exists. |
...
| Report approval and delivery | Matt's approval generates and stores the PDF, marks the submission completed, creates a seven-day client link, sends a Resend email, and returns a five-minute Matt link (`app/admin/assessments/actions.ts:798-945`). | **Live but not concurrency-safe.** The status guard is a read followed by an unconditional update, so two approval requests can both generate and email. Recipient selection is an unordered first portal user, and signed-link creation errors are ignored before dispatch. |
...
| Workflow Errors | Application-side failures are inserted into `workflow_errors`; Matt sees the 50 newest rows (`lib/supabase/dashboard.ts:402-417`; `app/admin/errors/page.tsx:8-74`). | **Partial and potentially misleading.** A database read failure is converted to an empty list, so the UI can say “No operational errors detected” when the log could not be read. Only seven of at least 22 written workflow names have tailored explanations (`lib/workflow-errors.ts:20-65`). n8n failures after a 2xx webhook acknowledgement never enter this table. |

### Automation findings that must not be filed as new QA bugs

- The three client-form n8n emails are already confirmed broken; executions `1` and `2` are the evidence. Do not repeat client writes merely to reproduce them.
- The separate assessment n8n email is already confirmed blocked by the application secret mismatch and exposed by an unauthenticated live webhook. Do not fire the public webhook as a test.
- `scripts/verify-n8n.ps1` is stale and unsafe for production verification: it still sends all 11 old Switch payloads, says the assessment call needs no authentication, and can create duplicate customer emails from obsolete n8n branches (`scripts/verify-n8n.ps1:1-37`; `scripts/verify-n8n.ps1:59-160`). `docs/n8n/email-templates.md` describes the same obsolete all-email-through-n8n design (`docs/n8n/email-templates.md:1-35`).
...
| n8n | Three client-form activity notices and a separate Matt-submitted assessment notice. It does not generate AI content, PDFs, or ordinary customer email. | **Partial/broken:** the authenticated general webhook is live, but its three current mail nodes cannot send. The assessment event is blocked in the application while its n8n webhook is publicly callable. | `lib/notifications/client-form-events.ts:28-72`; `lib/notifications/dispatch.ts:238-297`; `app/admin/assessments/actions.ts:428-475`; see **Live n8n audit** above. |
...
| n8n | The authenticated client-form webhook is live, but both recorded executions failed because the three current mail nodes use `888 / Matt` as the To value and never enrich `client_id`. The separate assessment event is blocked because its application secret is absent, while the live n8n endpoint itself has no authentication. Neither workflow has retries or an error workflow, both acknowledge before the Gmail node completes, and the shared audit key can read the live general webhook secret from retained execution data (`lib/notifications/dispatch.ts:238-297`; `app/admin/assessments/actions.ts:428-475`; see **Live n8n audit**). |
...
| Fix the three n8n client-form branches: real admin recipient, client-name enrichment, downstream failure alerting, and a controlled replay test | Dependable client activity notices | **Required; the live authenticated webhook currently accepts events and then fails.** |
| Protect the assessment n8n webhook and set the matching application secret | Safe Matt-submitted assessment notice | **Required; the application blocks its event while the live endpoint is public.** |
| Rotate the n8n API key and general webhook secret together, enable MFA, remove confirmed-unused mail credentials, and apply the available bug-fix update | Close the confirmed partner-automation credential exposure | **Required before handover; retained failed executions expose the current webhook secret to the shared API key.** |
