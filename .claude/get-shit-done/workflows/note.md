...
- **Global scope**: `C:/dev/Antigravity/888 Safety/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/dev/Antigravity/888 Safety/.claude/notes/`)
...
2. Glob `C:/dev/Antigravity/888 Safety/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/dev/Antigravity/888 Safety/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/dev/Antigravity/888 Safety/.claude/notes/` — works in any directory
